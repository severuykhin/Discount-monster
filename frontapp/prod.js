...
const prodPath     = './../backend/web/statics';
const layout       = './../backend/frontend/views/layouts/main.php';
 ...
 * @param {*} jsName - New js-build name
 * @param {*} cssName - New css-build name
 */
function replaceNames(path, jsName, cssName) {
	let layoutContent = fs.readFileSync(path, 'utf8');
	layoutContent = layoutContent.replace(
		/<!--!scripts-->[\s\S]+?<!--\/scripts-->/,
		`<!--!scripts--><script type="text/javascript" src="/statics/${jsName}"></script><!--/scripts-->`
	);
	layoutContent = layoutContent.replace(
		/<!--!styles-->[\s\S]+?<!--\/styles-->/,
		`<!--!styles--><link href="/statics/${cssName}" rel="stylesheet" type="text/css"><!--/styles-->`
	);
